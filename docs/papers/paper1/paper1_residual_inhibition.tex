...
\author{Jorge Sim\~ao}
\date{Paper 1 research draft --- Cycle A and Cycle B1--B5 results, August 2026}
\begin{document}\maketitle
\begin{abstract}
Behavioral scores underdetermine how a language model uses depth. Cycle A of this first experimental test of the Paper~0 dual-selection framework found that five-seed tiny causal decoders learned maximum and minimum goals but not modulo-10 sum. Every block was positively useful on the learned tasks; the sole replicated negative-utility cell occurred on the failed task and showed no later repair. Gates saturated near open and did not improve quality. Cycle B1 computed an exhaustive residual atlas; B2 separated self-attention (SA) and feed-forward (FF) writes; and B3 found no joint vector-plus-task repair in 12,150 trajectories. B4 trained 25 depth/capacity runs. At width 64, mean accuracy rose from .890 at depth 4 to .948 at depth 16 while crude effective-block fraction fell from 1.00 to .775, but the strict competence criterion failed. B5 repaired the task ecology using five high-identifiability value-selection goals. All five tasks were competent, all ten pairs were matched, and 13 pair/layer/component utility contrasts replicated; minimum layer 8 also had negative block and FF utility. Ordinary-residual goal decodability peaked mid-depth then declined. These results establish competence-matched task-conditioned computation and justify a targeted B6 gating test, while repair, strong interference, and competence-preserving depth slack remain unsupported.
\end{abstract}
...
Cycle B1--B5 expands what is known without rewriting that null. The residual carrier changes more smoothly across depth than candidate writes, exact SA/FF intervention shows a shallow causal asymmetry, and B3 finds no joint repair. B4's deeper models add localized negative and near-zero sublayers but fail the strict all-condition slack criterion. B5 supplies the missing competence control: five tasks and all ten pairs are matched, 13 task-conditioned utility contrasts replicate, and minimum layer 8 has negative block and FF utility. Ordinary-residual goal decodability peaks mid-depth, consistent with distributed goal information but not a distinct controller. These results support task-conditioned residual computation and justify targeted B6 gating; they still do not establish repair, strong interference, or a quality--compute benefit. Pretrained B7--B9 remain architecture- and resource-gated.
