...
\author{Jorge Sim\~ao}
\date{Paper 1 research draft --- Cycle A and Cycle B1--B4 results, August 2026}
\begin{document}\maketitle
\begin{abstract}
Behavioral scores underdetermine how a language model uses depth. Cycle A of this first experimental test of the Paper~0 dual-selection framework found that five-seed tiny causal decoders learned maximum and minimum goals but not modulo-10 sum. Every block was positively useful on the learned tasks; the sole replicated negative-utility cell occurred on the failed task and showed no later repair, so strong interference was not supported. Matched scalar gates saturated near open and did not improve quality, and an explicit goal latent underperformed pooled ordinary residuals for predicting future utility. Cycle B1 computed an exhaustive residual atlas over all 30 stored Cycle A checkpoints. B2 separated self-attention (SA) and feed-forward (FF) writes with exact native parity: SA had greater causal utility in seven of eight learned-task/layer cells, while both sublayers were positively useful. B3 measured token/depth stationarity and 12,150 intervention trajectories; no perturbation showed both vector recovery and task-gap recovery. B4 then trained 25 controlled depth/capacity runs. At width 64, mean accuracy rose from .890 at depth 4 to .948 at depth 16 while the crude effective-block fraction fell from 1.00 to .775, but the registered all-condition competence criterion failed. Two minimum-task FF cells nevertheless had replicated negative utility, one late SA cell was reliably near zero, and no joint repair appeared. These results motivate task-ecology validation and conditionally reopen gating without establishing interference or competence-preserving depth slack.
\end{abstract}
...
Cycle B1--B4 expands what is known without rewriting that null. Across all stored Cycle A models, the residual carrier changes more smoothly across depth than the candidate writes, and early anti-alignment coexists with positive learned-task utility. Exact SA/FF intervention shows a shallow causal asymmetry, while B3 finds no joint repair. B4's deeper models add localized evidence for selection pressure: two minimum-task FF cells have replicated negative utility and one late SA cell is reliably near zero. Yet no B4 perturbation repairs jointly, the strict all-condition competence rule fails, and the matched-parameter narrow/deep control underperforms. The results therefore reopen conditional gating but do not establish interference or depth slack. B5 must determine whether task-conditioned roles replicate under a larger competence-matched ecology before B6 tests evidence-selected gates; pretrained B7--B9 remain architecture- and resource-gated.
