...
Cycle B1--B2 expands what is known without rewriting that null. Across all stored models, the residual carrier changes more smoothly across depth than the candidate writes, and early anti-alignment coexists with positive learned-task utility. Exact SA/FF intervention then shows a real causal asymmetry: SA is more useful than FF in seven of eight learned-task/layer cells, while both remain positively useful. SA writes are also less similar across layers than FF writes. This supports sublayer specialization but not interference, intra-block repair, computational slack, or competence-matched goal-conditioned routing. Those remaining questions stay assigned to B3--B9, with gating reopened only if redundancy, negative utility, task-conditioned specialization, or replicated repair is first observed.
