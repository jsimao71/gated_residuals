...
\author{Jorge Sim\~ao}
\date{Paper 1 research draft --- E1--E7 status, August 2026}
\begin{document}\maketitle
\begin{abstract}
Behavioral scores underdetermine how a language model uses depth. This paper is the first experimental test of the Paper~0 dual-selection framework. We ask whether Transformer residual blocks refine an evolving solution, contribute complementary information, or sometimes damage a task-conditioned trajectory that later blocks repair. E1 measures causal block utility and repair in tiny decoder-only Transformers; E2 holds content fixed while changing distributed goal evidence; E3 compares matched baseline and gated residual models. A shared goal latent is secondary, and hard skipping is conditional on a robust E1--E3 signal. We preregister a released, closely matched Qwen3-based baseline/headwise-gated pair as an external probe, but do not report pretrained results because the exact checkpoint did not complete a parity forward under the available memory. Geometry is descriptive, ablation is functional, and the term \emph{strong interference} is reserved for replicated negative utility with later repair. E1--E3 did not support interference or selective scalar gating, E4's goal latent lost to ordinary residual summaries, and E5's combined gates failed to suppress the independently labeled negative cell.
\end{abstract}
