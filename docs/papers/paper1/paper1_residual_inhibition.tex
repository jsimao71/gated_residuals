...
\author{Jorge Sim\~ao}
\date{Paper 1 research draft --- Cycle A and Cycle B1--B3 results, August 2026}
\begin{document}\maketitle
\begin{abstract}
Behavioral scores underdetermine how a language model uses depth. Cycle A of this first experimental test of the Paper~0 dual-selection framework found that five-seed tiny causal decoders learned maximum and minimum goals but not modulo-10 sum. Every block was positively useful on the learned tasks; the sole replicated negative-utility cell occurred on the failed task and showed no later repair, so strong interference was not supported. Matched scalar gates saturated near open and did not improve quality, and an explicit goal latent underperformed pooled ordinary residuals for predicting future utility. Cycle B1 then computed an exhaustive residual atlas over all 30 stored Cycle A checkpoints. B2 separated self-attention (SA) and feed-forward (FF) writes with exact native parity: SA had greater causal utility in seven of eight learned-task/layer cells, while both sublayers were positively useful and no competence-matched task profile replicated. B3 measured token/depth stationarity and 12,150 intervention trajectories. Some correct/incorrect contrasts concentrated in late FF statistics, but none of the perturbations showed both vector recovery and task-gap recovery. These results support shallow-regime sublayer specialization while continuing to reject repair, interference, and goal-conditioned selection claims. Capacity expansion, task-ecology repair, and pretrained replication remain prospective.
\end{abstract}
...
Cycle B1--B3 expands what is known without rewriting that null. Across all stored models, the residual carrier changes more smoothly across depth than the candidate writes, and early anti-alignment coexists with positive learned-task utility. Exact SA/FF intervention shows a real causal asymmetry: SA is more useful than FF in seven of eight learned-task/layer cells, while both remain positively useful. B3 adds depth- and token-stability structure and a late-FF correctness candidate, but none of 12,150 perturbations shows joint vector and task-gap repair. This supports sublayer specialization but not interference, repair, computational slack, or competence-matched goal-conditioned routing. Those remaining questions stay assigned to B4--B9, with gating reopened only if redundancy, negative utility, task-conditioned specialization, or replicated repair is first observed.
