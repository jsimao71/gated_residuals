...
\section{Cycle C---learnability and conditional-computation preregistration}
Cycle C preserves all Cycle A/B results and moves the empirical question beyond an inhibition-only framing: how does a neural system allocate the strength, location, type, and depth of computation as task demands change? The registered hierarchy separates dense candidate transformations, soft amplitude control, hard execution selection, and conditional assembly through depth and expert routing. A small soft gate is not a binary route, and neither implies avoided FLOPs unless execution is actually bypassed.

The first stage holds architecture, data, optimizer, schedule, seeds, steps, and training tokens fixed while comparing dense residuals, static learned layer scales, dynamic block gates, and separate SA/FF gates. Learning-curve AUC and steps/tokens/wall time to competence are primary; final accuracy is insufficient. Later stages independently vary task difficulty and minimum depth, sparsity pressure, Mixture-of-Depths, projected head contributions, and a controlled MoD--MoE factorial before expanding the pretrained task ecology. Ordinary Qwen and small Llama/Gemma residual science is not blocked on the larger gated-Qwen checkpoint; that checkpoint remains a separate resource-conditional test.

The prospective Cycle C hypotheses are that dynamic conditioning can improve learning speed beyond static scaling on some regimes; explicit sparsity pressure can produce a quality--active-compute frontier; MoD more directly changes executed depth than soft gates; MoD and MoE can provide complementary routing axes; projected head writes reveal organization hidden by attention matrices; and router decisions encode causal utility beyond update magnitude only under the strong computational-selection interpretation. Nulls are retained, including static scaling matching dynamic gates, monotonic quality loss under sparsity, or no utility information beyond magnitude.
...
\subsection{B9: conditional Gemma-family stop}
B9 required both an end-to-end Qwen comparison and a completed Llama replication. Neither prerequisite was available: B7 lacked real gated-checkpoint parity and B8 contained only its eligibility decision. B9 therefore selected and downloaded no Gemma checkpoint, ran zero forwards, and produced no Gemma or cross-family metrics. As with B8, this is a documented upstream stop rather than an architecture-family result.

\section{Cycle C---Learnability and Computational Allocation}
\subsection{C1: fixed-budget learning curves}
C1 trained 20 depth-16, width-64 models: dense residual, static learned layer scale, dynamic sigmoid block gate, and separate sigmoid SA/FF gates, each at seeds 11, 22, 33, 44, and 55. Every run used the same five-task B5 ecology, data order by paired seed, AdamW settings, batch size, 24 epochs, 456 optimizer steps, and 924,168 non-padding training tokens. The static scale is an unconstrained layer scalar initialized at exactly one; a shared-weight test confirms exact step-zero parity with the dense model. Dynamic gates retain the B6 sigmoid parameterization initialized at $\sigma(2)$. Validation was measured every ten steps, the frozen test split was evaluated only after the fixed final state, and all threshold failures remain explicit nulls.
...
\toprule
Variant & Curve AUC & Paired $\Delta$ AUC & Test accuracy & Steps to 90\% & Steps to 99\% \\
\midrule
Dense & .782 [.770,.795] & --- & .992 [.990,.995] & 186 & 226 (5/5) \\
Static scale & .782 [.770,.795] & .0004 [-.0012,.0020] & .991 [.989,.993] & 184 & 236 (5/5) \\
Block gate & .780 [.764,.794] & -.0024 [-.0082,.0063] & .991 [.987,.993] & 190 & 235 (4/5) \\
SA/FF gates & .785 [.773,.798] & .0034 [-.0020,.0093] & .993 [.991,.995] & 186 & 212 (5/5) \\
\bottomrule
\end{tabular}
\caption{C1 five-seed fixed-budget results. Intervals are seed-bootstrap 95\% intervals; AUC differences are paired by seed. The block-gate 99\% mean is conditional on the four seeds that reached the threshold.}
\end{table}

No dynamic variant reliably improved learning-curve AUC over dense residuals, and static scaling was indistinguishable from both at this resolution. Mean steps to 80/90/95\% competence were respectively 174/186/192 for dense, 172/184/192 for static scale, 176/190/200 for the block gate, and 172/186/192 for separate SA/FF gates. At 99\%, the apparent SA/FF advantage is not a registered paired effect and terminal accuracy is saturated; one block-gate seed never crossed 99\% validation accuracy. Rounded mean non-padding tokens to 90\% were 377k, 373k, 385k, and 377k in the same order. Mean wall time to 90\% was 40.5, 38.8, 41.1, and 45.3 seconds, although these timings include validation and diagnostic capture and are not pure throughput benchmarks.

The optimization diagnostics show real amplitude changes without a learnability result. Mean gradient norms were .665 dense, .664 static, .647 block-gated, and .610 SA/FF-gated; mean parameter-update norms were .110, .108, .109, and .104. Final candidate/effective residual-update norms were 3.91/3.91, 3.92/3.97, 4.15/3.39, and 4.06/3.27. Corresponding mean applied multipliers were 1.000, 1.010, .805, and .762. Thus learned gates substantially attenuate candidate writes, but C1 does not show that this allocation improves optimization or final capacity under the easy, high-identifiability regime.

\begin{figure}[ht]
\centering
\includegraphics[width=.98\linewidth]{../../../artifacts/cycle_c/c1_learning_curves/figures/c1_learning_curves.pdf}
\caption{C1 validation learning curves against fixed non-padding token exposure, five-seed AUC distributions, and saturated frozen-test accuracy. Shading spans the observed seed range; no dynamic-gate paired AUC interval excludes zero.}
\end{figure}

C1 therefore preserves the null that final accuracy alone cannot justify gating and adds a stronger learnability null under matched training exposure. It does not establish that dynamic conditioning is useless: task difficulty and minimum necessary depth were held fixed at a regime that all variants mastered. C2 consequently tests whether any optimization or effective-capacity distinction emerges as difficulty rises or depth becomes limiting, without changing the C1 result after seeing that sweep.

\subsection{C2: goal-cue difficulty and depth}
C2 added 120 fixed-budget runs and reused the 20 identical C1 high-identifiability depth-16 runs. Four goal-cue regimes were frozen: direct high-identifiability wording, indirect medium wording, oblique low wording, and a separate mixed-cue condition cycling all three forms. The first three form the preregistered directness ladder; mixed cues measure wording diversity and are not treated as a fourth ordinal directness level. Every run retained the five B5 value-selection tasks, paired seeds, 24 epochs, and fixed optimizer schedule. Reliable competence required mean validation accuracy at least .90 and at least four of five seed accuracies at least .90.

All four variants reliably learned high, medium, and low single-cue regimes. At low identifiability, mean validation/test accuracy was .980/.964 dense, .980/.965 static-scale, .972/.971 block-gated, and .973/.963 SA/FF-gated. Mean normalized learning-curve AUC ranged only from .751 to .755. Thus the maximum reliably learned point on the directness ladder was the oblique low-identifiability wording for every variant; dynamic conditioning did not extend that boundary.

Mixed cues created the actual capacity/optimization stress. At depth 16, mean validation accuracy fell to .858 dense, .869 static, .865 block-gated, and .849 SA/FF-gated; only one, two, two, and one seeds respectively reached .90. Paired mixed-cue AUC differences versus dense were $-.00002$ [95\% interval $-.00091,.00081$] for static, $-.00210$ [$-.00732,.00219$] for block gates, and $.00026$ [$-.00472,.00532$] for SA/FF gates. No dynamic variant reliably improved convergence or final validation quality.
...
\begin{tabular}{lrrrr}
\toprule
Variant & Depth 4 & Depth 8 & Depth 12 & Depth 16 \\
\midrule
Dense & .742 (1/5) & .884 (2/5) & .873 (2/5) & .858 (1/5) \\
Static scale & .759 (1/5) & .901 (4/5) & .889 (2/5) & .869 (2/5) \\
Block gate & .768 (1/5) & .893 (3/5) & .879 (2/5) & .865 (2/5) \\
SA/FF gates & .756 (1/5) & .891 (3/5) & .862 (2/5) & .849 (1/5) \\
\bottomrule
\end{tabular}
\caption{C2 mixed-cue mean validation accuracy with number of individually competent seeds. The registered cell criterion requires mean $\geq.90$ and at least 4/5 competent seeds.}
\end{table}

Depth was strongly non-monotonic. Only static scaling at depth 8 crossed the literal cell rule, with .901 mean validation and four competent seeds; its paired validation improvement over dense was .0167 [.0007,.0300]. However, its mean frozen-test accuracy was lower than dense (.841 versus .848), the paired test interval crossed zero, and the same static variant failed at depths 12 and 16. No other variant qualified at any depth. We therefore record depth 8 as the \emph{lowest qualifying static-scale cell}, not a minimum necessary depth: no variant has a depth after which every tested deeper model remains reliably competent.

\begin{figure}[ht]
\centering
\includegraphics[width=.98\linewidth]{../../../artifacts/cycle_c/c2_difficulty_depth/figures/c2_difficulty_depth.pdf}
\caption{C2 goal-cue and mixed-depth results. Dashed lines mark the .90 validation threshold. Mixed-cue performance is non-monotonic in depth and highly seed-sensitive.}
\end{figure}

C2 sharpens the C1 null. Dynamic gates neither extend the single-cue difficulty boundary nor solve mixed-cue optimization. Static scaling produces one fragile validation-qualified depth-8 cell, consistent with an optimization reparameterization rather than task-conditioned selection: it does not persist at greater depth or improve frozen-test accuracy reliably. C3 therefore introduces explicit sparsity pressure to test a different claim---quality versus active computation---without treating C2 as evidence that gates improve effective capacity.
...
Cycle B1--B9 expands what is known without rewriting that null. The residual carrier changes more smoothly across depth than candidate writes, exact SA/FF intervention shows causal asymmetry, and B3 finds no joint repair. B4 adds localized negative and near-zero sublayers but fails the strict slack criterion. B5 supplies the missing competence control: five tasks and all ten pairs match, 13 task-conditioned utility contrasts replicate, and minimum layer 8 has negative block/FF utility. B6 shows that deep learned gates are functionally consequential under forced-open and shuffled interventions, unlike Cycle A's saturated gates. However, native quality merely matches baseline, gate--utility alignment is weaker than gate--magnitude alignment, the negative cell persists, and no candidate computation is skipped. B7 validates exact real-model instrumentation and reproduces SA/FF causal asymmetry in standard Qwen3-0.6B, but the matched gated checkpoint remains unevaluated under an explicit resource stop. B8 and B9 consequently record conditional Llama and Gemma stops with zero models or examples. The combined result supports task-conditioned residual dynamics, soft amplitude modulation, and portable measurement semantics, not repair, strong interference, a quality--compute benefit, or gated/cross-family transfer.

Cycle C1 asks a different question of that same mechanism: whether amplitude allocation changes how efficiently competence is learned. Across 20 fixed-budget runs, static and dynamic controls all reached essentially the same saturated final quality. Separate SA/FF gates had the largest AUC point estimate, but its paired interval included zero; whole-block gating was slightly slower on average and also inconclusive. Gates reduced effective relative to candidate update norms, yet did not reliably improve AUC, threshold steps, tokens, or wall time. The next evidential boundary is therefore difficulty and minimum depth, not a post-hoc claim that gating wins on the C1 ecology.

C2 finds that boundary in cue diversity rather than single-cue directness. Every architecture reliably learned each fixed wording regime, including oblique low-identifiability prompts, while all four failed the mixed-cue depth-16 criterion. Depth did not behave as a monotone capacity axis: the sole qualifying cell was static scaling at depth 8, and that validation advantage neither survived at greater depth nor produced a reliable frozen-test gain. Dynamic gates consequently show no learnability or effective-capacity advantage through C2. The next stage tests whether an explicit sparsity objective can instead create a quality--active-compute frontier.
