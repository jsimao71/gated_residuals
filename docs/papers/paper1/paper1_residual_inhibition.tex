...
\author{Jorge Sim\~ao}
\date{Paper 1 research draft --- E1--E8 first-cycle results, August 2026}
\begin{document}\maketitle
\begin{abstract}
Behavioral scores underdetermine how a language model uses depth. This paper reports the first experimental cycle of the Paper~0 dual-selection framework. Five-seed tiny causal decoders learned maximum and minimum goals but not modulo-10 sum. Every block was positively useful on the learned tasks; the sole replicated negative-utility cell occurred on the failed task and showed no later repair, so strong interference was not supported. Same-content goal contrasts did not replicate between competence-matched tasks. Matched scalar gates saturated near open and did not improve quality, a shared goal latent predicted future utility worse than pooled ordinary residuals, and their combination did not suppress the independently labeled negative cell. Accordingly, hard skipping and the PRA factorial failed their preregistered evidence gates. A pinned pretrained headwise-gated checkpoint did not complete a real parity forward under available memory, so no pretrained metric is reported. The first cycle therefore supports distributed residual refinement in learned tasks while falsifying the proposed interference, goal-control, and selective-gating interpretations in this setting.
\end{abstract}
...
\section{Conclusion}
The first cycle answers its bounded causal question with a constrained null. On tasks the tiny decoders learned, all four residual blocks contributed positive causal probability utility, favoring distributed refinement. The only negative cell belonged to a chance-level task and lacked repair. Competence-matched counterfactual goals did not yield replicated utility changes. Scalar gates remained nearly open, forcing them open barely changed behavior, and neither gate-only nor goal-conditioned gating improved quality or selected independently useful computation. The shared latent also failed the stronger future-utility test against ordinary residual summaries.

These results weaken residual interference, a distinct shared task-control state, mechanistic inhibition, and sufficient activation for this model/task regime. They do not show that those mechanisms are impossible, nor do they evaluate the released pretrained gated-attention model. A second cycle would first require a competence-matched task ecology, thresholds frozen before its new test split, and a mechanism that exhibits nontrivial selection on validation data. Until those conditions hold, expanding hard-skip, pretrained, or PRA-factorial claims would convert exploratory tuning into unsupported confirmation. The defensible contribution of this cycle is the falsifiable instrumentation, the replicated refinement map, and the preserved negative outcomes.
